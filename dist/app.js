...
var D3Svg = /** @class */ (function () {
    function D3Svg(options) {
        ...
        var wrapperContainerStyle = options.wrapperContainerStyle;
        /**
        ...
        this.svg = select(this.wrapperContainer)
            .append("svg")
            .attr("width", width)
            .attr("height", height);
 ...
 * 数据相关
 */
csv$1('data/Week01/admissions.csv', function (err, data) {
    ...
    var BAR_PADDING = 2;
    var BAR_WIDTH = (boxSize.width - 100) / data.length - BAR_PADDING;
    var bar = new BarChart({
        data: data,
        container: svgContainer,
    });
    var counts = data.map(function (admission) {
        return admission.Count;
    });
    var linearScale = linear().domain([0, Number(max(counts)) * 2]).range([0, boxSize.height]);
    bar
        .x(function (d, i) {
        return (i + 1) * 40 + 10;
    })
        .y(function (d) {
        return boxSize.height - linearScale(d.Count);
    })
        .width(BAR_WIDTH)
        .height(function (d, i) {
